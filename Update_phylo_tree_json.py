...
""" Update phylogenetic tree within a json file
...
def read_tree():
    ...
    tree = Tree("/path/to/tree.nwk", format = 1)
    return tree
...
def read_my_dict():
    ...
    """
    my_dict_new = benedict.from_json("/path/to/file.json",keypath_separator=None)
...
def update_dict(dic,path_name):
    return dic[path_name].update({"new_key":"new_value"})
...
def conv_bene_write(n1):
    ...
    dic_to_save.to_json("/path/to/updated_file.json", indent = 1)
